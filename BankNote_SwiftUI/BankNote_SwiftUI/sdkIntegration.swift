...
class sdkIntegration: NSObject, ObservableObject, VIDVLivenessDelegate {
    
    static let shared = sdkIntegration()
...
//    @Published var vidvLivenessBuilder: VIDVLivenessBuilder?
    ...
    var LivenessDelegate: LivenessResultDelegate?
    ...
    func startLiveness(transactionFrontId : String){
        ...
        generateAccessToken { [weak self] token, error in
            ...
            if let token = token {
                ...
                var builder = VIDVLivenessBuilder()
                builder = builder.setBundleKey(self.bundleKey)
...
//                    .withoutCloseEyes()
                ...
                // debug prints: addresses
                  if let builderObj = builder as AnyObject? {
                      debugPrint("builder addr:", Unmanaged.passUnretained(builderObj).toOpaque())
                  }
                  debugPrint("self addr:", Unmanaged.passUnretained(self).toOpaque())
                ...
                if let windowScene = UIApplication.shared.connectedScenes.first as? UIWindowScene,
                   let rootVC = windowScene.windows.first(where: { $0.isKeyWindow })?.rootViewController ?? windowScene.windows.first?.rootViewController {
                    builder.start(vc: rootVC, livenessDelegate:self)
